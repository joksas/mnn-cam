...
def I_to_y(I: tf.Tensor, k_V: float, max_weight: float, G_off: float, G_on: float) -> tf.Tensor:
    ...
    """
    I_total = I[:, 0::2] - I[:, 1::2]
    y = I_total_to_y(I_total, k_V, max_weight, G_off, G_on)
    return y


def I_total_to_y(
    I_total: tf.Tensor, k_V: float, max_weight: float, G_off: float, G_on: float
) -> tf.Tensor:
    ...
    Returns:
        Outputs of shape `p x n` of a synaptic layer implemented using memristive crossbars.
    """
    k_G = _compute_k_G(max_weight, G_off, G_on)
    k_I = _compute_k_I(k_V, k_G)
    y = I_total / k_I
    return y


def _compute_k_G(max_weight: float, G_off: float, G_on: float) -> float:
